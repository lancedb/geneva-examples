...
def run(
    ...
    output_column: str = typer.Option("pose", help="Output pose-skeleton column."),
    include_hand: bool = typer.Option(True, help="Include hand keypoints."),
    include_face: bool = typer.Option(True, help="Include face keypoints."),
    ...
    num_cpus: float = typer.Option(2.0, help="CPUs per model task."),
    num_gpus: float | None = typer.Option(None, help="GPUs per task (default 0.25)."),
    ...
    concurrency: int = typer.Option(32, help="Backfill concurrency."),
    ...
    from geneva_examples.udfs.openpose import OPENPOSE_RUNTIME_PIP, build_openpose_udf
    ...
    manifest = (
        GenevaManifest.create_pip(f"frame-openpose-{uuid.uuid4().hex[:6]}")
        .pip(OPENPOSE_RUNTIME_PIP)
        .build()
    )
