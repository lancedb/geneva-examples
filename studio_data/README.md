...
Files in `images/`, `videos/`, and `audio/` are git-ignored (only `.gitkeep`
is tracked), so your media never gets committed. `input.csv` ships with a few
example rows — replace it with your own.
